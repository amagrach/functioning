...
outsp <- data.frame(Site_id = NA, Round = NA, #year= NA,
                species=NA,  norm_degree = NA,strength= NA, nestedra= NA,weigh_betweeness  = NA, 
                weigh_closeness=NA, d=NA, c=NA, z=NA, specificity=NA)
webs <- list()
for(i in 1:length(sites)){
  temp <- subset(sp, Site_ID == sites[i])
  for(j in 1:length(rounds)){
    temp2 <- subset(temp, Round == rounds[j])
        ...
        #p1<-plotweb(web)
         ...
         ntw<-try(networklevel(web), TRUE)
         n <- nrow(out)
          webs[[n]] <- web
          mod<-try(computeModules(web), TRUE)
          cz<-try(czvalues(mod, level="lower", weighted=TRUE), TRUE)
          spec<-getspe(web)
          
          n2 <- nrow(outsp)
          out[n + 1,1] <- as.character(sites[l])
          out[n + 1,2] <- as.character(rounds[j])
          out[n + 1,3:11] <- c(ntw[3:4],ntw[9], ntw[12:13], ntw[15:18])   
          
          
        
        outsp[n2+seq(nrow(web)),1] <- as.character(sites[l])
        outsp[n2+seq(nrow(web)),2] <- as.character(rounds[j])
        outsp[n2+seq(nrow(web)),3] <- try(as.character(rownames(web)), TRUE)
        outsp[n2+seq(nrow(web)),4:9] <- try(c(spntw$`lower level`[2:3], spntw$`lower level`[5],
              spntw$`lower level`[12], spntw$`lower level`[14],
              spntw$`lower level`[20]), TRUE)
        outsp[n2+seq(nrow(web)),10:11] <- try(c(cz$c,
                                              cz$z), TRUE)
        outsp[n2+seq(nrow(web)),12] <- spec
