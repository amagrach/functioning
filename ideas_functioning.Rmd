...
AINHOA OTHER IDEAS: We could look at in the future
...
1) How does network structure, and in particular two aspects (nestedness and functional complementarity), that represent redundancy (and thus potentially resilience to species loss) and complementarity in niche (and thus potentially increased reproductive success through niche differentiation, specialization and increased pollen deposition of same species) link to ecosystem function (mean fruit and seed set) at the community level?

2) How does the node position of a plant species within a network affect ecosystem functioning (here reproductive success)? Apparent competition with other plant species? Pollinator diversity visiting?

3) How does network structure affect equity in functioning?
...
pl2
# 
# interact_plot(m2.see, pred = "functional.comp.poll", modx = "nodf.song", interval = TRUE, plot.points = TRUE, int.type = "confidence",  facet.modx = TRUE, x.label = "Functional complementarity", y.label = "Mean seed number/fruit",
...
mycols["Apis"] <- "yellow"
  ...
  pl.si<- ggplot(si,  aes(x=Pollinator_genus, y=pollen_tubes, fill=Pollinator_genus))+  
    geom_bar(stat="identity")  + scale_fill_manual(values=mycols) + theme (legend.position= "none") #+ 
...
El control tiene valores super altos, esta bien?? COntrol es el negro, open el gris