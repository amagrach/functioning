...
Title: Impact of interaction network structure on community-level plant reproduction success  
or "Interaction network structure maximizes community-level plant reproduction success via [niche complmentariety?]()"
...
[IB: this intro is brilliant!]()

# METHODS

_Plant pollinator interactions_

Our study was conducted in SW Spain within the area of influence of Doñana National Park (Figure 1?). Here, we surveyed 16 Mediterranean woodland patches, located within a XXX m2 area and with an average distance of XX km between them (min= XX, max= XX). [SHOULD WE SAY SOMETHING ABOUT THE MATRIX SURROUNDING THESE SITES OR THE TYPE OF VEGETATION PRESENT IN THEM??? IB: I don't think is relevant. ASK CURRO ABOUT THE NUMBERS ABOVE.]() Each site was surveyed 7 times during the flowering season of 2015 (from February to May) following a 100-m x 2 m transect for 30 mins. Along each transect, we identified all plant species and recorded all the flower visitors [IS IT JUST BEES AND HOVERFLIES? IB: NO, ALL VISITORS!]() that legitimately visited them. Floral visitors (from now on referred to as pollinators) that could not be identified in the field were captured, stored and identified in the laboratory by FdPM with the aid of experts in the different taxonomic groups (see acknowledgements). All surveys were done under similar weather conditions, avoiding windy or rainy days. 

_Plant reproductive success_

Within each site, we marked between 2 and 12 individuals (6.49 +- 2.37) of 1 [NB: Which site has only one species!]() to 6 plant species (4.06 +- 1.69), depending on the availability and presence of flowers during the sampling events. For each individual, we counted the number of flowers at each sampling point and recorded fruit set, the number of seeds per fruit (for 2-36 fruits, 11.17 +- 6.84) and average seed weight per fruit at the end of the season. Our survey included a total of 19 different plant species across our 16 sites. 
...
At the community level, we followed the same logic as the one presented at the species level. Thus, we also calculated two network metrics providing complementary non-redundant information. In this case we focused on (i) nestedness and (ii) functional complementarity. Nestedness is the property by which specialists interact with a subset of the species that generalists interact with ([Bascompte et al 2003 PNAS]). Although there is an ongoing debate in the literature, some studies have found that nested networks are more stable and resilient to perturbations because nestedness implies a certain level of redundancy and thus the possibility for functional replacement in the case of species loss (REF). [We used the widely accepted NODF metric to calculate Nestedness]() However, plant reproductive success depends on the delivery of conspecific pollen and thus of a certain level of specialization or niche divergence (reviewed in [@Brosi2016]). Yet at present we do not know how the interplay between redundancy and complementarity in species roles determines reproductive success. [To calculate functional complmentariey we used ...](). All network analysis were perfeormed using bipartite package.

Many network attributes vary with network size and complexity [@Bluthgen2006]. In the case of nestedness, we know it can be affected by network size and connectance (REF). An approach that is often used to correct for this, is to use null models and to compare null-model corrected nestedness values across different networks. However, this approach has been recently shown to present the same issues, as z-scores also change with network size and connectance [@Song2017]. Given the dependence of nestedness on network size and connectance, we followed the advice of these researchers by using a normalized value of nestedness, NODFc, calculated as XXXX, where XXXXX [INCLUDE FORMULA AND DEFINE CONCEPTS], which is independent of network size and thus comparable across different networks [@Song2017]. All network metrics were calculated using package bipartite [@Dormann2009].
...
At the community level, we upscaled our species-level analyses. In this case, model 1 included total pollinator diversity and the total number of visits received by all plants within the community as explanatory variables. Model 2, in turn, added information on community structure by including nestedness and functional complementarity as explanatory variables. As response variable we had the average reproductive success for the whole community (i.e., average fruitset, average number of seeds per fruit previously scaled for each species, and average fruit and seed weight [IB: also scaled?]()). We thus, had a single value per site and no random effects are needed in this case.

Average values of reproductive success at the community level can be driven to a large extent by a single plant species. Yet, what will determine the persistence of a diverse plant community, is the presence of some sort of “equity” in reproductive success across the whole community. We therefore calculated a measure of equity in reproductive success at the community level as the proportion of plant species within a site that is at 50% of the maximum value of fruitset recorded for that plant species within our sampling. We repeated this using 25% and 75% thresholds. We then used the same framework as before and fit the same models 1 and 2 as the ones used at the community level, but using equity in reproductive success as the response variable. [Here is unclear how the proportion is calculated. This is: average fruit set per species is > 50%? Or as we discussed a more coplex option can be: % of species which > 50 % of individuals had seed set > 50%. Now that I see this I would only used if we gain a lot, otherwise is too complicated. Also, I removed species present is only one (or two) sites for this analysis, because the % was forces to be 1. If you kept this in your analysis, should be explained]().

We used information criterion methods to find the best model for each case and selected the ones with the lowest Akaike Information Criterion (AIC) value. Differences in AIC values < 2, were not considered enough to select one model over the other [@Burnham2011].

_Simulating the effect of community structure as sample size increases_

Finally, we tested whether the importance of community structure in explaining differences in reproductive success increases with the number of plant species being considered. We expect that when only one plant species is considered, then the importance of community structure will be negligible, while we expect this importance to increase as more plant species are considered (up to a maximum number of 6 species which is the maximum we have measured in our study at a particular site).
...
At the species level, our results show that in the case of fruitset, both model 1 and model 2 are equally good in explaining the variability observed when accounting for model complexity (i.e.,$\Delta AIC<2$), and therefore we cannot select one over the other. Given that model 2 has a greater number of variables which penalize for the added complexity, we will thus comment results only for model 2, which in the case of fruitset shows a positive effect of the diversity of species visiting a focal plant species (measured using the species´normalised degree, Fig. XX) as well as a positive effect of the centrality of the focal plant on its fruitset. 
...
When evaluating the effect of differences in community composition and structure for equity in reproductive success across the different individuals within a species and site we find that both model 1 and model 2 are equally good (i.e.,$\Delta AIC<2$) and thus we cannot select one over the other. Specifically, regarding the results for model 2, we find a significant and positive effect of nestedness for equity between individuals of a species (Table X, Fig. X). This effect that we find when using a 50% threshold of productivity (i.e., when looking at plant species that are producing 50% of the maximum fruitset observed for the area), is however not maintained when using 25 and 75% thresholds. In particular, when using a 25% threshold we find model 1 is the best model and none of the variables considered are able to explain the differences observed. Contrastingly in the case of the 75% threshold, we find again that both model 1 and 2 are equally plausible but in this case we find a positive effect of functional complementarity between pollinators for equity in fruitset across individuals within a species.
[this is interesting, as selfing rates I expect to be ~ 40%... In any case I am confused now on this calculation]()

```{r, include=TRUE, echo = FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.height=5}

#EQUITY IN INDIVIDUALS WITHIN SPECIES
...
fr.max<-aggregate(.~ Plant.sp, f3, FUN=max)

f3$fruit.max<-fr.max$tot.fruitset[match(f3$Plant.sp, fr.max$Plant.sp)]

f3$fruit.prop<-f3$tot.fruitset/f3$fruit.max
f5<-subset(f3, f3$fruit.prop>0.5)
...
f6<- aggregate(no_rows ~ Site_ID + Plant.sp, f5, length)
head(f6)
...
f3$no_rows<-rep(1, nrow(f3))
f3b<- aggregate(no_rows ~ Site_ID + Plant.sp, f3, length)
head(f3b)


f3b$j<- paste(f3b$Site_ID, f3b$Plant.sp)
f6$j<- paste(f6$Site_ID, f6$Plant.sp)



f6$total.plant.numb<-f3b$no_rows[match(f6$j, f3b$j)]
...
#fit two models
m1.eq<-glmer(equity ~  species.poll.sc + tot.visits.sc + (1|Site_ID), family="binomial", data=f6)
...
m2.eq<-glmer(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc+ (1|Site_ID), family="binomial", data=f6)
...
library(remef)
y_partial_eq <- remef(m2.eq, fix = c("species.poll.sc", "tot.visits.sc", "functional.comp.poll.sc"), ran = "all", inverse = F)
y_partial_eq_inv<-exp(y_partial_eq)/(1+exp(y_partial_eq))
 

pl.th_ <-ggplot(f6,  aes(x=nodf.song.sc, y=y_partial_eq_inv ))+      
  geom_point(size=4, color="black") + geom_smooth(method=lm, size=1.5, se=TRUE, color = "black", alpha=0.2) +  ylim(0,1.1) +
  xlab("Nestedness") +      
...
#now 75% threshold
#head(f3)
f5.75<-subset(f3, f3$fruit.prop>0.75)
f5.75$no_rows<-rep(1, nrow(f5.75))
head(f5.75)

#for each site here is the number of sps whose fruitset exceeds 50% of maximum fruitset observed in the whole study area
f6.75<- aggregate(no_rows ~ Site_ID + Plant.sp, f5.75, length)
head(f6.75)
...
f3b$j<- paste(f3b$Site_ID, f3b$Plant.sp)
f6.75$j<- paste(f6.75$Site_ID, f6.75$Plant.sp)



f6.75$total.plant.numb<-f3b$no_rows[match(f6.75$j, f3b$j)]
f6.75$equity<-f6.75$no_rows/f6.75$total.plant.numb

#join this with fruitset data used previously

f6.75$nodf.song<-out.site$nodf.song[match(f6.75$Site_ID, out.site$Site_id)]
f6.75$functional.comp.poll<-out.site$functional.comp.poll[match(f6.75$Site_ID, out.site$Site_id)]
f6.75$species.poll<-out.site$species.poll[match(f6.75$Site_ID, out.site$Site_id)]
f6.75$tot.visits<-f.agg$tot.visits[match(f6.75$Site_ID, f.agg$Site_ID)]

#scale all variables

f6.75$nodf.song.sc <- scale(f6.75$nodf.song, center = T, scale = T)
f6.75$functional.comp.poll.sc <- scale(f6.75$functional.comp.poll, center = T, scale = T)
f6.75$species.poll.sc <- scale(f6.75$species.poll, center = T, scale = T)
f6.75$tot.visits.sc <- scale(f6.75$tot.visits, center = T, scale = T)

#fit two models
m1.eq.75<-glmer(equity ~  species.poll.sc + tot.visits.sc + (1|Site_ID), family="binomial", data=f6.75)

summary(m1.eq.75)


m2.eq.75<-glmer(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc+ (1|Site_ID), family="binomial", data=f6.75)
#nich estimate not overlaping zero: 0.6178     0.4354, second higher effect size!
summary(m2.eq.75)
...
AICtab(m1.eq.75, m2.eq.75) #Almost equal... Again, for prediction, go with the simple, for mechanistic understanding, look into niche partitioning. 
library(remef)
y_partial_eq.75 <- remef(m2.eq.75, fix = c("species.poll.sc", "tot.visits.sc", "nodf.song.sc"), ran = "all", inverse = F)
y_partial_eq.75_inv<-exp(y_partial_eq.75)/(1+exp(y_partial_eq.75))
 

pl.th_75 <-ggplot(f6.75,  aes(x=functional.comp.poll.sc, y=y_partial_eq.75_inv ))+      
  geom_point(size=4, color="black") + geom_smooth(method=lm, size=1.5, se=TRUE, color = "black", alpha=0.2) + ylim(0,1.1) +
  xlab("Functional complementarity") +      
  ylab("Equity in fruitset") + theme_bw() + 
  theme(text = element_text(size = 20), axis.text=element_text(size=rel(1.2)), axis.title=element_text(size=rel(1.3), face="bold")) 
...
library(cowplot)
plot_grid(pl.th_, pl.th_75, labels = c("A", "B")) 

#now 25% threshold
f5.25<-subset(f3, f3$fruit.prop>0.25)
f5.25$no_rows<-rep(1, nrow(f5.25))
head(f5.25)

#for each site here is the number of sps whose fruitset exceeds 50% of maximum fruitset observed in the whole study area
f6.25<- aggregate(no_rows ~ Site_ID + Plant.sp, f5.25, length)
head(f6.25)
...
f3b$j<- paste(f3b$Site_ID, f3b$Plant.sp)
f6.25$j<- paste(f6.25$Site_ID, f6.25$Plant.sp)



f6.25$total.plant.numb<-f3b$no_rows[match(f6.25$j, f3b$j)]
f6.25$equity<-f6.25$no_rows/f6.25$total.plant.numb

#join this with fruitset data used previously

f6.25$nodf.song<-out.site$nodf.song[match(f6.25$Site_ID, out.site$Site_id)]
f6.25$functional.comp.poll<-out.site$functional.comp.poll[match(f6.25$Site_ID, out.site$Site_id)]
f6.25$species.poll<-out.site$species.poll[match(f6.25$Site_ID, out.site$Site_id)]
f6.25$tot.visits<-f.agg$tot.visits[match(f6.25$Site_ID, f.agg$Site_ID)]

#scale all variables

f6.25$nodf.song.sc <- scale(f6.25$nodf.song, center = T, scale = T)
f6.25$functional.comp.poll.sc <- scale(f6.25$functional.comp.poll, center = T, scale = T)
f6.25$species.poll.sc <- scale(f6.25$species.poll, center = T, scale = T)
f6.25$tot.visits.sc <- scale(f6.25$tot.visits, center = T, scale = T)

#fit two models
m1.eq.25<-glmer(equity ~  species.poll.sc + tot.visits.sc + (1|Site_ID), family="binomial", data=f6.25)

summary(m1.eq.25)


m2.eq.25<-glmer(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc+ (1|Site_ID), family="binomial", data=f6.25)
#nich estimate not overlaping zero: 0.6178     0.4354, second higher effect size!
summary(m2.eq.25)
...
```
Figure X. Partial residual plots showing the effect of a) nestedness and b) functional complementarity on equity in reproductive success between individuals of a species within a site, measured as the proportion of individuals producing at least 50% or 75% of maximum fruitset observed for the study area respectively. Dots represent values for each species within a site (N=63)

Contrastingly, in the case of equity in average reproductive success across species within a site we find that model 1, the one including only simple visitation metrics is the best one, and we find none of the variables explain the differences observed at any of the thresholds considered (25, 50 and 75%).
...
fr.max.comm<-aggregate(.~ Plant.sp, f4, FUN=max)

f4$fruit.max<-fr.max.comm$mean.fruitset[match(f4$Plant.sp, fr.max.comm$Plant.sp)]

f4$fruit.prop<-f4$mean.fruitset/f4$fruit.max

f4b<-subset(f4, f4$fruit.prop>0.5)
f4b$no_rows<-rep(1, nrow(f4b))
head(f4b)

#for each site here is the number of sps whose fruitset exceeds 50% of maximum fruitset observed in the whole study area
f4c<- aggregate(no_rows ~ Site_ID, f4b, length)
head(f4c)
...
f4c$equity<-f4c$no_rows/f4c$total.plant.numb

#join this with fruitset data used previously

f4c$nodf.song<-out.site$nodf.song[match(f4c$Site_ID, out.site$Site_id)]
f4c$functional.comp.poll<-out.site$functional.comp.poll[match(f4c$Site_ID, out.site$Site_id)]
f4c$species.poll<-out.site$species.poll[match(f4c$Site_ID, out.site$Site_id)]
f4c$tot.visits<-f.agg$tot.visits[match(f4c$Site_ID, f.agg$Site_ID)]

#scale all variables

f4c$nodf.song.sc <- scale(f4c$nodf.song, center = T, scale = T)
f4c$functional.comp.poll.sc <- scale(f4c$functional.comp.poll, center = T, scale = T)
f4c$species.poll.sc <- scale(f4c$species.poll, center = T, scale = T)
f4c$tot.visits.sc <- scale(f4c$tot.visits, center = T, scale = T)

#fit two models
 m1.eq.comm<-glm(equity ~  species.poll.sc + tot.visits.sc, family="binomial", data=f4c)

 summary(m1.eq.comm)

 m2.eq.comm<-glm(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc, family="binomial", data=f4c)
summary(m2.eq.comm) 
...
head(f75b)
f75b$total.numb.plants<-f4d$no_rows[match(f75b$Site_ID, f4d$Site_ID)]
...
f75b$tot.visits.sc <- scale(f75b$tot.visits, center = T, scale = T)
 ...
 m1.eq.75<-glm(equity ~  species.poll.sc + tot.visits.sc, family="binomial", data=f75b)

 summary(m1.eq.75)

 vif(m1.eq.75)
 
m2.eq.75<-glm(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc, family="binomial", data=f75b)
...
head(f25b)
f25b$total.numb.plants<-f4d$no_rows[match(f25b$Site_ID, f4d$Site_ID)]
...
f25b$tot.visits.sc <- scale(f25b$tot.visits, center = T, scale = T)
 ...
 m1.eq.25<-glm(equity ~  species.poll.sc + tot.visits.sc, family="binomial", data=f25b)

 summary(m1.eq.25)

 vif(m1.eq.25)
 
m2.eq.25<-glm(equity ~  species.poll.sc + tot.visits.sc + nodf.song.sc + functional.comp.poll.sc, family="binomial", data=f25b)
...
AICtab(m1.eq.25, m2.eq.25) #as before both models are equally good.
...
CHECK THIS IN LIGHT OF NEW RESULTS!!
...
Because i do not expect the average to change as dramatically when considering more species. 
...
The relationship between community structure and ecosystem function has been long suggested, yet the specific mechanisms by which structure influences function remain elusive. Our results show that different aspects of community structure affect different dimensions of functioning. In particular, we find that the functional complementarity between pollinators, a measure of the overlap in the niches of different species in terms of plant coverage, has an important effect for direct measures of reproductive success (e.g., fruitset). However, we also find that nestedness, a measure of the redundancy in ecological functions between pollinators is important in determining a stable and equitable function across whole communities.

One of the first conclusions we can extract from the fact that in most cases both of the models we considered (i.e., the simple model based on visitation metrics and the more complex one including community structure metrics) were equally good, is that for simple predictive purposes, simple visitation metrics, such as the diversity of pollinators visiting a plant, might be enough [cite Garibaldi papers using them](). Yet, a mechanistic understanding of the underlying processes requires that we take into account measures of community structure. 

Consistent with previous studies in artificial [@Frund2013] and natural [@Fontaine2005] communities, we find that functional complementarity is very important in determining differences in reproductive outputs. Indeed, this reflects the fact that reproductive success in plant species requires of the delivery of conspecific pollen and thus of a certain degree of specialization amongst pollinator species on a particular plant resource in order to avoid the negative effects of inter-specific pollen deposition (e.g., pollen loss [@Flanagan2009] or interference with conspecific pollen [@Morales2008]). 

Nevertheless, we also find that redundancy in pollinator functions is also important in determining a certain stability in the pollination service across a larger number of individuals within a plant species. Indeed, our results also support previous accounts of the importance of nestedness for plant-pollinator communities [@Bascompte2003] as it ensures that reproductive success is distributed amongst a larger number of individuals, thus ensuring the maintenance of a greater genetic diversity in natural populations. This aspect of reproductive, which is seldom taken into account, is however of great importance, as genetic diversity will ensure the resilience of populations to further change. 

Paragraph somewhere about the importance of how you measure functioning (average vs equity).

Our findings show that the analysis of natural communities of interacting species using network analysis not only represents an ideal way of visualizing and grasping the complexity present, but also represents a manner of mechanisitically understanding differences observed across individuals and/or species. 

Future studies should focus on the long-lasting effects of community structure for plant fitness, by evaluating the consequences of different community structures for heritability. 
