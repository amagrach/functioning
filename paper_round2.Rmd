...
bibliography: library.bib
editor_options: 
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
...
=======
Our study was conducted in SE Spain within the area of influence of Doñana National Park. Here, we surveyed 16 Mediterranean woodland patches. Each site was surveyed 7 times following a 100-m x 2 m transect for 30 mins. Along each transect we identified all plant species and the pollinators that legitimately visited them. In addition, within each site we evaluated fruit set, seed number per fruit and seed weight for several individuals belonging to 15 [in abstract you menton 19]() plant species. Not all plant species were present in all transects.
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
...
The data for all rounds was pooled to construct one plant-pollinator interaction network per site, for which we calculated a set of relevant network metrics. At the species level, we focused on: degree (i.e., number of pollinator species visiting a particular plant), total number of visits recieved, centrality and average niche overlap between a focal species and other plant species in the community. We fit three models, one including only the diversity of pollinator species and the total number of species. One including only metrics related to a plant´s position in the community and a third one combining both sets of models and used information criterion methods to find the best model. We included plant species nested within site as random effect to account for non-independence of several individuals measured for the same plant species.
[is seed set scaled across plants?]()

__Network level__

We followed the same logic at the community level, where we evaluated the importance of pollinator diversity and total visitation compared to that of network structure. Including in this case nestedness and functional complementarity as metrics. In the case of nestedness it is known that it can be biased by network size. One approach to correct for that is to use null-model and compare the null-corrcted netwedness values across different networks. However, this approach has the problem that the corrected z-scores values still depend on network size and connectance as shown by this recent paper by Song et al. What these authors propose is to use a normalized value of nestedness, and to control by the effect of connectance and network size, thus obtaining a new value of nestedness NODFc which is independent of network size and thus comparable across different networks. 
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
...
=======
Within our transects we recorded 57 species of plants and 277 species of pollinators (HAY QUE MIRAR MORFOESPECIES! [IB: yes, but most should be identified to species level, to unique morphospecies. Ask Curro.]()).
Our sampling completeness analyses revealed that with our survey we were able to capture 18-62% of pollinator species (average= 35%), 47-98% for plant species (average= 78%) and 13-41% for plant-pollinator links (average=27%), in line with that obtained with other studies (e.g., Chacoff et al), yet slightly smaller in the case of pollinators given the great diversity of pollinators found in the Mediterranean region in particular and in the our study area, a hotspot of insect diversity (?¿?).
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
...
head(f2)
#why this is done? visits whould be all in the same scale already? 
#If it is to correct for different atractiveness, you should scale as below (by plant)
#If it is to scale the predictors and make estimates comparable, you should do that to all predictors.
...
pairs.panels(f2[,c(31,27, 35, 37)]) 
 ...
 m1<-glmmTMB(tot.fruitset ~  norm_degree + tot.visits  + (1|Plant.sp/Site_ID) , family="gaussian", data=f2)
 summary(m1)
 #why (1|Site_ID) is repeated?? and plant?

 m2<-glmmTMB(tot.fruitset ~   weigh_closeness + niche.overlap  + (1|Plant.sp/Site_ID) , family="gaussian", data=f2)
 summary(m2)

 m3<-glmmTMB(tot.fruitset ~  norm_degree  + tot.visits + weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID), family="gaussian", data=f2)
 summary(m3)
...
#yes, but m3 is 0.6 AIC away... which is nothing... and m1 with just one random is almost equal too!
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
...
s2$tot.visits.sc<-scale(s2$tot.visits) #scale number of visits
...
library(dplyr)
s2_scaled <- s2 %>% group_by(Plant.sp) %>% mutate(mean.seedn = scale(mean.seedn, center = T, scale = T))


h<-hist(s2_scaled$mean.seedn)

 m1.seed<-glmmTMB(mean.seedn ~  norm_degree + tot.visits.sc +  (1|Plant.sp/Site_ID), family="gaussian", data=s2_scaled)
 summary(m1.seed)

 m2.seed<-glmmTMB(mean.seedn ~   weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID) , family="gaussian", data=s2_scaled)
 summary(m2.seed)

m3.seed<-glmmTMB(mean.seedn ~  norm_degree  + tot.visits.sc + weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID), family="gaussian", data=s2_scaled)
=======
head(s2)
s2$mean.seedn.sc<-((s2$mean.seedn-min(s2$mean.seedn))/(max(s2$mean.seedn)-min(s2$mean.seedn))) 
#This scaling is not working like that. 1) scale() will do that for you. 2) you need to scale each plant in turn.
library(dplyr)
s3 <- s2 %>% 
  group_by(Plant.sp) %>% 
  mutate(mean.seedn.sc = scale(mean.seedn, center = T, scale = T))
dim(s2)
dim(s3)
head(s3)

#FOR TESTING PUROSES I overwrite s2
s2 <- s3

h<-hist(s2$mean.seedn)
h2<-hist(s2$mean.seedn.sc)
 
 m1.seed<-glmmTMB(mean.seedn.sc ~  norm_degree + tot.visits.sc +  (1|Site_ID) + (1|Site_ID), family="gaussian", data=s2)
 summary(m1.seed)
#Ojo, AIC changes if the second (1|Site_ID) is removed.
 m2.seed<-glmmTMB(mean.seedn.sc ~   weigh_closeness + niche.overlap + (1|Site_ID) , family="gaussian", data=s2)
 summary(m2.seed)

m3.seed<-glmmTMB(mean.seedn.sc ~  norm_degree  + tot.visits.sc + weigh_closeness + niche.overlap + (1|Site_ID), family="gaussian", data=s2)
>>>>>>> 4a45bf7d61c1c851ab41328bfc825a68bf59ea6f
summary(m3.seed)


library(bbmle)
AICtab(m1.seed, m2.seed, m3.seed) #2 is better, but all of them are pretty bad...

#test for problems in residuals
res<-simulateResiduals(m2.seed)
plot(res, rank = TRUE)
#residuals good with scaled and gaussian

pl2<-ggplot(s2_scaled,  aes(x=weigh_closeness, y=mean.seedn, color=Plant.sp))+      
        geom_point(size=4) + geom_smooth(method=lm, size=1.5, se=TRUE, color = "black", alpha=0.2) + 
        geom_smooth(method=lm, linetype= "dashed", size=0.7, se=FALSE, alpha=0.8) +
        xlab("Closeness centrality") +      
...
#But this do not makes much sense, if they are scaled by species... see above.
...
h<-hist(s2$mean.seedweight)
 ...
 m1.seedw<-glmmTMB(mean.seedweight ~  norm_degree + tot.visits +  (1|Plant.sp/Site_ID) , family="poisson", data=s2)
 summary(m1.seedw)

 m2.seedw<-glmmTMB(mean.seedweight ~   weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID) , family="poisson", data=s2)
 summary(m2.seedw)

m3.seedw<-glmmTMB(mean.seedweight ~  norm_degree  + tot.visits.sc + weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID), family="poisson", data=s2)
summary(m3.seedw)


library(bbmle)
AICtab(m1.seedw, m2.seedw, m3.seedw) #again, very similar (and bad) models.
...
h<-hist(s2$mean.fruitweight)
 ...
 m1.fruitw<-glmmTMB(mean.fruitweight ~  norm_degree + tot.visits +  (1|Plant.sp/Site_ID) , family="poisson", data=s2)
 summary(m1.fruitw)

 m2.fruitw<-glmmTMB(mean.fruitweight ~   weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID) , family="poisson", data=s2)
 summary(m2.fruitw)

m3.fruitw<-glmmTMB(mean.fruitweight ~  norm_degree  + tot.visits + weigh_closeness + niche.overlap + (1|Plant.sp/Site_ID), family="poisson", data=s2)
summary(m3.fruitw)
...
```
[IB: I wonder if AIC is what we want here. Why compare models 1 and 2? One option is to present directly m3. Or m1 vs m3 maybe. I don't have a clear idea... skype?]
### Network-level
#### FRUITSET
...
library(bbmle)
AICtab(m1.fr, m2.fr, m3.fr) #m3 is as good!!!
##I hate models... m2 nodf is sig, m3 fc is sig!! WHY!!! colineality issues?
...
#seed number
#How mean seed number is calculated?? scaled first would be better, right?
...
#fruit weight
#How mean seed number is calculated?? scaled first would be better, right? At least if fruits vary in orders of magnitude.
...
library(bbmle)
AICtab(m1.eq, m2.eq, m3.eq) #m3 as good almost
...
#permanova... we need to control for the web, right? not sure what you mean here, random effect?
