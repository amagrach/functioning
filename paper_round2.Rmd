...
AICtab(m1.fruit.net, m3.fruit.net) #Almost as good. The take home message would be. If interested in predictions, go for simplicity. If interested in mechanisms, nich part is a mechanism to take into account.
...
pl2_ <-ggplot(f.agg,  aes(x=functional.comp.poll.sc, y=mean.fruitset)) +    
        geom_point(size=4, color="darkgrey") + geom_smooth(method=lm, size=1.5, se=TRUE, color = "black", alpha=0.2) + 
        xlab("Niche complementariety") +      
        ylab("Fruitset") + theme_bw()  + #theme (legend.position= "none") +
        theme(text = element_text(size = 20), axis.text=element_text(size=rel(1.2)), axis.title=element_text(size=rel(1.3), face="bold")) + labs(col="Plant species")

pl2_ #Well, moderate... without scaling it was more pronunced?
...
AICtab(m1.eq, m3.eq) #Almost equal... Again, for prediction, go with the simple, for mechanistic understanding, look into niche partitioning. 
...
colnames(combinatorials_all) <- c("SiteID", "Plant_number_level", "iteration", "equitativity", "niche")
...
for(k in 1:length(uniqueSite_ID)){ #sites
  ...
  colnames(combinatorials) <- c("SiteID", "Plant_number_level", "iteration", "equitativity", "niche")
...
scatter.smooth(combinatorials_all$equitativity ~ combinatorials_all$niche) #no relationship, good
...
for (i in unique(combinatorials_all$Plant_number_level)){
  ...
  m <- lm(temp$equitativity ~ temp$niche)
  plot(temp$equitativity ~ temp$niche) 
  abline(m)
  estimate[i] <- m$coefficients[2]
  }

plot(unique(combinatorials_all$Plant_number_level) , estimate, las = 1,
...
for (i in 1:length(unique(combinatorials_all$Plant_number_level))){ # over levels
  ...
  for(k in 1:100){ #iterations
    ...
    for(j in 1:length(unique(temp$SiteID))){ # over sites
      ...
      value[j,2] <- value_temp$equitativity
      value[j,3] <- value_temp$niche
    } #close sites
