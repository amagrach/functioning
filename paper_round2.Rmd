...
Ainhoa Magrach, Francisco de Paula [or Curro, ask him]() Molina, Ignasi Bartomeus
...
Our study was conducted in SW Spain within the area of influence of Doñana National Park (Figure 1). Here, we surveyed 16 Mediterranean woodland patches with an average distance of 7 km between them (min= 3 km, max= 46.5 km). Each site was surveyed 7 times during the flowering season of 2015 (from February to May) following a 100-m x 2 m transect for 30 mins. Along each transect, we identified all plant species and recorded all the flower visitors that legitimately visited them during each 30-min period. In addition, at each round we conducted 3 minutes of focal observations on 3 individuals belonging to the 19 most common species totalling X census [give mean per site?](Curro). In addition, we included some interactions between plant and pollinators that were not observed during the sampling but that were seen immediately after as some of these interactions are difficult so see and might be important to define network structure. Floral visitors (from now on referred to as pollinators) that could not be identified in the field were captured, stored and identified in the laboratory by FdPM with the aid of experts in the different taxonomic groups (see acknowledgements). All surveys were done under similar weather conditions, avoiding windy or rainy days. 
...
[See new paper by Benno simons updating Song method :(]()
...
At the species level, in the case of fruitset, our results show that model 2 is the best model explaining the variability observed. In this case, we find a positive effect of pollinator species diversity (measured using the species´normalised degree, Fig. 2) as well as a positive effect of the centrality of the focal plant on its fruitset (Fig. 2). [to what extent ND is a network level metric i.e. not a simple metric like number of links¿? I am playing devils advocate here.]() [ND is not a network metric here, it is a simple measure of diversity. I used ND because it was easy to extract from bipartite, but technically it is not one of the community metrics, that´s why it is also in model 1]()
...
rownames(Table1)<-c("(Intercept)", "Normalized degree", "Total number of visits", "Weighted closeness", "Niche overlap")
...
rownames(Table2)<-c("(Intercept)", "Normalized degree", "Total number of visits", "Weighted closeness", "Niche overlap")
...
Figure3<-ggplot(m2.seed@frame,  aes(x=m2.seed@frame$niche.overlap.sc, y=y_partial_norm_seed_inv, color= Plant.sp)) +    
  ...
  #geom_smooth(method=lm, linetype= "dashed", size=0.7, se=FALSE, alpha=0.8) +
  xlab("Niche overlap") +      
  ylab("Seeds per fruit") + theme_bw()  + theme (legend.position= "bottom") +
  theme(text = element_text(size = 18), axis.text=element_text(size=rel(1)), axis.title=element_text(size=rel(1.3), face="bold")) + 
...
rownames(tableS4)<-c("(Intercept)", "Normalized degree", "Total number of visits")
...
When evaluating the effect of differences in community composition and structure for equity in reproductive success across the different species within a community we find that model 1 is the best model for all the thresholds considered (50%, 25% and 75%). However, we only find a significant negative relationship between equity and pollinator species diversity using the 75% threshold (Fig. 6, Tables S8, S9, S10). 
...
The authors would like to thank Oscar Aguado, Javier ?? and XX for help in identifying pollinator species. In addition, XX. AM received funding from a Juan de la Cierva and Ikerbasque fellowships. IB acknowledges funding from MSC-PCIG14-GA-2013-631653 BeeFun Project.
...
kable(Table1, caption = "Table 1. Results of GLMM showing effect of simple visitation and community structure metrics on species-level fruitset based on best model selected.")
```
\newpage
...
kable(Table2, caption = "Table 2. Results of GLMM showing effect of simple visitation and community structure metrics on species-level number of seeds/fruit based on best model selected.")
```
\newpage
...
```{r, include=TRUE, echo = FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.height=6, fig.width=8}
...
kable(tableS8, caption="Table S8. Results of GLM showing effect of simple visitation metrics on equity in reproductive success across plant species within a community based on best model selected (50% threshold).")

kable(tableS9, caption="Table S9. Results of GLM showing effect of simple visitation metrics on equity in reproductive success across plant species within a community based on best model selected (75% threshold).")

kable(tableS10, caption="Table S10. Results of GLM showing effect of simple visitation metrics on equity in reproductive success across plant species within a community based on best model selected (25% threshold).")
```
