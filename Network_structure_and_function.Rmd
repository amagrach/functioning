...
m1.seed.w<-glmer(mean.seedw ~  pollspsperplant.sc + tot.visits.sc  + (1|Plant.sp:Site_ID) + (1|Site_ID), family="gaussian", data=s3)

summary(m1.seed.w)
 
 
m2.seed.w<-glmer(mean.seedw ~  pollspsperplant.sc + tot.visits.sc + weigh_closeness.sc + niche.overlap.sc  + (1|Plant.sp:Site_ID)+ (1|Site_ID), family="gaussian", data=s3)
...
tableS5<-round(coefs.m1.seed.w,digits=2)
rownames(tableS5)<-c("(Intercept)", "Pollinator richness", "Total number of visits")

#FRUIT WEIGHT


m1.fruit.w<-glmer(mean.fruitweight ~  pollspsperplant.sc + tot.visits.sc  + (1|Plant.sp:Site_ID)+ (1|Site_ID), family="gaussian", data=s3)

summary(m1.fruit.w)


m2.fruit.w<-glmer(mean.fruitweight ~  pollspsperplant.sc  + tot.visits.sc + weigh_closeness.sc + niche.overlap.sc  + (1|Plant.sp:Site_ID) + (1|Site_ID), family="gaussian", data=s3)
...
tableS6<-round(coefs.m1.fruit.w,digits=2)
rownames(tableS6)<-c("(Intercept)", "Pollinator richness", "Total number of visits")

FigureS3<-plot_summs(m2, m2.seed, m1.fruit.w, m1.seed.w, level=0.95, scale = TRUE, model.names = c("Fruit set", "Number of seeds", "Fruit weight", "Seed weight"), legend.title = "Response variables", cex.axis=2)

```
 For all other measures of reproductive success considered (i.e., fruit and seed weight), model 1 showed the best fit. However, none of the variables included within our model explain the differences observed (Tables S5-S6). 
...
FigureS4A<-plot_model(m2.fruit.net, title="Fruit set")
```
...
FigureS4B<-plot_model(m2.seed.net, title="Number of seeds")
```
...
#save table with results for paper
tableS7A<-round(summary(m1.fruitw.net)$coef[,1:3], digits=2)#ch
rownames(tableS7A)<-c("(Intercept)", "Pollinator species diversity", "Total number of visits")#end of paper
...
AICtab(m1.fruitw.net2, m2.fruitw.net2)



#save table with results for paper
tableS7B<-round(summary(m1.fruitw.net2)$coef[,1:3], digits=2)#ch
rownames(tableS7B)<-c("(Intercept)", "Pollinator species diversity", "Total number of visits")#end of paper
...
#save table with results for paper
tableS8A<-round(summary(m1.seedw.net)$coef[,1:3], digits=2)#ch
rownames(tableS8A) <-c("(Intercept)", "Pollinator species diversity", "Total number of visits")#end of paper
...
AICtab(m1.seedw.net2, m2.seedw.net2) #m1 is better


#save table with results for paper
tableS8B<-round(summary(m1.seedw.net2)$coef[,1:3], digits=2)#ch
rownames(tableS8B) <-c("(Intercept)", "Pollinator species diversity", "Total number of visits")#end of paper
...
FigureS5<-plot_grid(pltestb, pltest2.sub, labels = c("A", "B"), nrow=1)  



FigureS4C<-plot_summs(m1.fruitw.net, m1.seedw.net, level=0.95, scale = TRUE, model.names = c("Fruit weight", "Seed weight"), legend.title = "Response variables")

#FigureS4<-plot_grid(FigureS4A, FigureS4B, FigureS4C, labels = c("A", "B", "C"), nrow=3)  
```

  Contrastingly, in the case of weight variables (fruit and seed weight), in both cases we find that the best model is model 1, i.e., that only including simple visitation metrics (R^2 = `r round(r2.fruitw1[2], 2)` in the case of fruit weight and `r round(r2.seedw1[2], 2)` in the case of seed weight). Here, we find a consistent positive effect of site-level pollinator richness for both weight descriptors (Tables S7A-S8A, Fig. 4). This effect is maintained even after removing a site that has a particularly large pollinator richness value (Tables S7B-S8B, Fig. S5). 
...
**Figure 2**. Partial residual plots showing the effect of the single predictor which best explains the variability in species-level reproductive success. A) Shows the effect of plant species centrality on the fruit set of each of the plant species considered and B) shows the effect of plant niche overlap on the average number of seeds per fruit. Dots represent each of the individuals sampled for each species within each site.
...
```
**Figure S4.** Forest plots showing 95% confidence intervals for all coefficients included in site-level models, A) fruit set, B) number of seeds per fruit and C) fruit and seed weight. 
...
tableS7A<-round(summary(m1.fruitw.net)$coef[,1:3], digits=2)
...
tableS7B<-round(summary(m1.fruitw.net2)$coef[,1:3], digits=2)
rownames(tableS7B)<-c("(Intercept)", "Pollinator richness", "Total number of visits")
...
```{r, include=TRUE, echo = FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.height=5}
#save table with results for paper
tableS8A<-round(summary(m1.seedw.net)$coef[,1:3], digits=2)
rownames(tableS8A) <-c("(Intercept)", "Pollinator richness", "Total number of visits")

kable(tableS8A, caption="Results of GLM showing effect of simple visitation metrics on A) site-level average seed weight based on best model selected and B) the same analysis removing one site that has a particularly large pollinator richness value to test whether this point might be driving the relationship.")
```

```{r, include=TRUE, echo = FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.height=5}
#save table with results for paper
tableS8B<-round(summary(m1.seedw.net2)$coef[,1:3], digits=2)
rownames(tableS8B) <-c("(Intercept)", "Pollinator richness", "Total number of visits")

kable(tableS8B, caption="Results of GLM showing effect of simple visitation metrics on A) site-level average seed weight based on best model selected and B) the same analysis removing one site that has a particularly large pollinator richness value to test whether this point might be driving the relationship.")
```
