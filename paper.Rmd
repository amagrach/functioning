...
#setwd("C:/Users/ainhoa.magrach/Dropbox/Beefun/Beefun/functioning") #use rstudio projects and avoid wd issues.
#I am uploading a Rproj. just cleick there to open it the next time.
...
f2$tot.fruitset<-(f2$Fruit_Yes +  f2$Fruit_preyed + f2$Fruit_dispersed)/(f2$Fruit_Yes +f2$Fruit_No + f2$Fruit_preyed + f2$Fruit_dispersed) #why not use a binomial response and use both the numerator and the denominator? mmm... this will bias the estimates for plants producing larger number of seeds. 


s2<-subset(s, Notes %nin% wordList)
s2$tot.seedset #this variable do not exists
...
f.agg2<-na.omit(f.agg) #you are ommiting here also NA's on SE, which is consevartive.
...
m1.fr3<-glmmTMB(mean.fruitset ~ nodf.song + functional.comp.poll +   (1|Plant.sp), family="gaussian", data=f.agg2)
summary(m1.fr3)
...
pl1 #This suggests random slopes... 
...
#also analyze effect of diversity

m.th.poll.div<-glmmTMB(no_rows ~ species.poll , family="gaussian", data=f6)
summary(m.th.poll.div)

m.th.pl.div<-glmmTMB(no_rows ~ species.pl , family="gaussian", data=f6)
summary(m.th.pl.div)


pl.th3<-ggplot(f6,  aes(x=species.pl, y=no_rows))+      
        geom_point(size=4) + geom_smooth(method=lm, size=1.5, se=TRUE, color = "black", alpha=0.2) + 
        xlab("Plant species diversity") +      
        ylab("Evenness in fruitset") + theme_bw() + 
        theme(text = element_text(size = 20), axis.text=element_text(size=rel(1.2)), axis.title=element_text(size=rel(1.3), face="bold")) 
...
mp2$class[mp2$mean.fruitset<0.75]<-"mid"
mp2$class[mp2$mean.fruitset<0.5]<-"low"
mp2$class[mp2$mean.fruitset>0.75]<-"high"
...
#lots of empty positions. why?

#test mcount(M = m, six_node = TRUE, normalise = TRUE)
test <- mcount(M = webs[[1]], six_node = FALSE, normalise = TRUE)
sum(test$nodes) #74 positions

#see one position model
plot(mp2$mean.fruitset ~ mp2$m2)


pca <- prcomp(mp2[,3:48]) #there are two cases that break everything...
biplot(pca)
summary(pca)


#Notes, before doing the NMDS wes should remove imposible positions (e.g. 1,3,5,7,... for pollinators.)
# NMDS
example_NMDS <- metaMDS(vegdist(mp2[,paste0("m",1:46)], method = "euclidian"), k=2, distance = "bray", trymax = 100)
mds.fig <- ordiplot(example_NMDS, type = "none")
colors=c(rep("yellow",class_f["low"]), rep("darkorange2",class_f["mid"]),rep("purple",class_f["high"]))
treat <- mp2$class
...
points(mds.fig, "sites", pch = 19, col = "darkorange3", select = mp2$class == 
         "low")
points(mds.fig, "sites", pch = 19, col = "yellow", select = mp2$class == 
         "mid")
...
#permanova... we need to control for the web, right?

adonis(mp2[,3:48]~ mean.fruitset, data=mp2, method = "euclidian", strata = mp2$web)
